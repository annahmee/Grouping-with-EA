import pandas as pd
# import grouping.algorithm.tools.res_initial as res_initial
...
import math
from grouping.algorithm.tools import resultToCSV
import copy
from grouping.algorithm.tools import fit_fun
# import grouping.algorithm.tools.fit_fun as fit_fun
import random
from openpyxl import load_workbook
from openpyxl.styles import Alignment
import matplotlib.pyplot as plt
import numpy as np
import grouping.algorithm.tools.plot_fun
import grouping.algorithm.tools.resultToCSV
import grouping.algorithm.tools.Diversity
...
def begin(n=4, x=2, n_groups=50, teachers=4, accuracy_level=2, clash_teacher=[], same_teacher=[], rd=0.5, alpha=0.9, save = 0.8,no_dabian=[],filedir=r'./algorithm/input_data/',filename='file.xlsx'):
    '''
    ...
    data = pd.read_excel(io=filedir+filename,dtype=object)
    data22 = pd.read_excel(io=filedir+filename)
    ...
    #plt.savefig(filedir[:-21]+"figure/"+str(time.time()).replace('.','')+".svg",format="svg")
    #plt.savefig(filedir[:-21]+"figure/"+str(time.time()).replace('.','')+".png")
    ...
    diversity_list = []
    diversity_list.append(grouping.algorithm.tools.Diversity.Diversity(lzs))
    ...
    for iter_num in range(500):
        ...
        for index, lz in enumerate(lzs):
            ...
            if cost_2 > fit_fun.fit_all(g_best, id_teacher, id_score, score_scale, n, same_teacher):
                ...
                a,b = grouping.algorithm.tools.fit_fun.show_fit(g_best, id_teacher, id_score, score_scale, n, same_teacher)
                print("fit:")
                print(a,b)
            # 计算本轮出现最好粒子
    ...
    # //n个组的分布图
    #grouping.algorithm.tools.plot_fun.plot_scale(g_best, "GA_MU", id_score)
    # 将结果写到csv
    data,filename=grouping.algorithm.tools.resultToCSV.to_CSV(g_best, "GA_MU", id_score, id_teacher,id_name,id_theme,filedir=filedir)
    # 画出迭代过程中多样性的图
    #grouping.algorithm.tools.plot_fun.plot_diversity(diversity_list, "GA_MU")
    # 画出迭代过程中适应度的图
    #grouping.algorithm.tools.plot_fun.plot_fitness(g_best_list, "GA_MU")
...
if __name__ == '__main__':
    a,b,c = begin(n=6, accuracy_level=2,teachers=4,filename='2020_data.xlsx')
    print(b[-1])
    print(b)
